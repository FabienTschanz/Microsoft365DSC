...
[DscResource()]
class IntuneMobileAppsSystemAppAndroid : M365DSCResourceBase
{
    [DscProperty(Mandatory)]
    [System.ComponentModel.Description('The app identifier. Cannot be changed after creation.')]
    ...
    [MSFT_DeviceManagementSystemMobileAppAssignment[]] $Assignments
    ...
    [IntuneMobileAppsSystemAppAndroid] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneMobileAppsSystemAppAndroid]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration for the Intune Mobile Apps System App for Android with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.DisplayName -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Mobile Apps System App for Android with Id {$($this.Id)}"
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Mobile Apps System App for Android with DisplayName {$($this.DisplayName)}."
    ...
    [void] Set()
    {
        ...
        Write-Verbose -Message "Setting configuration of the Intune Mobile Apps System App for Android with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        {
            Write-Verbose -Message "Creating an Intune Mobile Apps System App for Android with DisplayName {$($this.DisplayName)}"
...
class MSFT_DeviceManagementSystemMobileAppAssignment : MSFT_DeviceManagementMobileAppAssignment
{
    [DscProperty()]
...
class MSFT_DeviceManagementSystemMobileAppAssignmentSettings : MSFT_DeviceManagementMobileAppAssignmentSettings
{
    [DscProperty()]
    ...
    [DscProperty()]
    [System.ComponentModel.Description('The prioritization of automatic updates for this app assignment. The possible values are: default, postponed, priority, unknownFutureValue.')]
