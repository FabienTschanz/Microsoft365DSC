...
[DscResource()]
class IntuneWindowsAutopilotDeploymentProfileAzureADHybridJoined : M365DSCResourceBase
{
    [DscProperty()]
    [System.ComponentModel.Description('The Autopilot Hybrid Azure AD join flow will continue even if it does not establish domain controller connectivity during OOBE.')]
    [System.Nullable[System.Boolean]] $HybridAzureADJoinSkipConnectivityCheck
    ...
    [MSFT_MicrosoftGraphoutOfBoxExperienceSettings] $OutOfBoxExperienceSettings
    ...
    [IntuneWindowsAutopilotDeploymentProfileAzureADHybridJoined] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneWindowsAutopilotDeploymentProfileAzureADHybridJoined]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune Windows Autopilot Deployment Profile Azure AD Hybrid Joined with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.DisplayName -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Windows Autopilot Deployment Profile Azure AD Hybrid Joined with Id {$($this.Id)}"
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Windows Autopilot Deployment Profile Azure AD Hybrid Joined with DisplayName {$($this.DisplayName)}"
