...
[DscResource()]
class IntuneMobileAppsMicrosoft365SuiteMacOS : M365DSCResourceBase
{
    [DscProperty(Key)]
    ...
    [IntuneMobileAppsMicrosoft365SuiteMacOS] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneMobileAppsMicrosoft365SuiteMacOS]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration for the Intune Mobile Apps Microsoft365 Suite for macOS with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.DisplayName -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Mobile Apps Microsoft365 Suite for macOS with Id {$($this.Id)}"
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Mobile Apps Microsoft365 Suite for macOS with DisplayName {$($this.DisplayName)}."
            ...
            Write-Verbose -Message "An Intune Mobile Apps Microsoft365 Suite for macOS with Id {$($resolvedId)} and DisplayName {$($this.DisplayName)} was found"
    ...
    [void] Set()
    {
        ...
        Write-Verbose -Message "Setting configuration of the Intune Mobile Apps Microsoft365 Suite for macOS with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        {
            Write-Verbose -Message "Creating an Intune Mobile Apps Microsoft365 Suite for macOS with DisplayName {$($this.DisplayName)}"
    ...
    hidden [IntuneMobileAppsMicrosoft365SuiteMacOS] AsResult([System.Object] $Values)
    {
        if ($Values -is [IntuneMobileAppsMicrosoft365SuiteMacOS])
        {
            return $Values
        }

        $result = [IntuneMobileAppsMicrosoft365SuiteMacOS]::new()
