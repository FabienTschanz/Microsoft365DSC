...
[DscResource()]
class IntuneAntivirusExclusionsPolicyLinux : M365DSCResourceBase
{
    [DscProperty()]
    ...
    [IntuneAntivirusExclusionsPolicyLinux] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneAntivirusExclusionsPolicyLinux]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune Antivirus Exclusions Policy for Linux with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.Name -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Antivirus Exclusions Policy Linux with Id {$($this.Id)}"
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Antivirus Exclusions Policy Linux with Name {$($this.DisplayName)}."
            ...
            Write-Verbose -Message "An Intune Antivirus Exclusions Policy Linux with Id {$($resolvedId)} and Name {$($this.DisplayName)} was found"
    ...
    [void] Set()
    {
        ...
        {
            Write-Verbose -Message "Creating an Intune Antivirus Exclusions Policy Linux with Name {$($this.DisplayName)}"
    ...
    hidden [IntuneAntivirusExclusionsPolicyLinux] AsResult([System.Object] $Values)
    {
        if ($Values -is [IntuneAntivirusExclusionsPolicyLinux])
        {
            return $Values
        }

        $result = [IntuneAntivirusExclusionsPolicyLinux]::new()
