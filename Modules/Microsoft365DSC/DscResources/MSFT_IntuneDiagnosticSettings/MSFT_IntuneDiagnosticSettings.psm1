...
[DscResource()]
class IntuneDiagnosticSettings : M365DSCResourceBase
{
    [DscProperty(Key)]
    ...
    [IntuneDiagnosticSettings] Get()
    {
        # Declared up front: assigned conditionally below, which class methods reject.
        $nullResult = $null
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneDiagnosticSettings]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of Intune Diagnostic Settings for Name $($this.Name)"
    ...
    [void] Set()
    {
        ...
        Write-Verbose -Message "Setting configuration of Intune Diagnostic Settings for Name $($this.Name)"
        ...
        if ($this.Ensure -eq 'Present')
        {
            ...
            $null = Invoke-AzRestMethod -Uri "$((Get-MSCloudLoginConnectionProfile -Workload Azure).ManagementUrl)providers/microsoft.intune/diagnosticsettings/$($this.Name)?api-version=2017-04-01-preview" `
                -Method PUT `
                -Payload $payload
        }
        # REMOVE
