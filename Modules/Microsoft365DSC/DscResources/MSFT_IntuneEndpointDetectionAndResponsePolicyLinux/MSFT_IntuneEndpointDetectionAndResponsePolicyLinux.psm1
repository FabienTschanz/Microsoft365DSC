...
[DscResource()]
class IntuneEndpointDetectionAndResponsePolicyLinux : M365DSCResourceBase
{
    [DscProperty(Key)]
    ...
    [IntuneEndpointDetectionAndResponsePolicyLinux] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneEndpointDetectionAndResponsePolicyLinux]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune Endpoint Detection And Response Policy Linux with Id {$($this.Id)} and Name {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.Name -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Endpoint Detection And Response Policy Linux with Id {$($this.Id)}"
                    ...
                    if (-not [System.String]::IsNullOrEmpty($this.DisplayName))
                    {
                        ...
                        {
                            throw "Duplicate Intune Endpoint Detection And Response Policy Linux named $($this.DisplayName) exist in tenant"
                        }
                    }
                }
                #endregion
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Endpoint Detection And Response Policy Linux with Name {$($this.DisplayName)}."
            ...
            Write-Verbose -Message "An Intune Endpoint Detection And Response Policy Linux with Id {$($resolvedId)} and Name {$($this.DisplayName)} was found"
    ...
    [void] Set()
    {
        ...
        {
            Write-Verbose -Message "Creating an Intune Endpoint Detection And Response Policy Linux with Name {$($this.DisplayName)}"
