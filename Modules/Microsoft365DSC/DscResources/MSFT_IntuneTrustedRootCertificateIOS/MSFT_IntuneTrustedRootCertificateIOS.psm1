...
[DscResource()]
class IntuneTrustedRootCertificateIOS : M365DSCResourceBase
{
    [DscProperty()]
    ...
    [IntuneTrustedRootCertificateIOS] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneTrustedRootCertificateIOS]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune Trusted Root Certificate Policy for iOS with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.DisplayName -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    $getValue = Get-MgBetaDeviceManagementDeviceConfiguration -All -Filter "DisplayName eq '$($this.Displayname -replace "'", "''")' and isof('microsoft.graph.iosTrustedRootCertificate')" -ErrorAction SilentlyContinue
                }
                #endregion

                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "No Intune Trusted Root Certificate Policy for iOS with Id {$($this.Id)} was found"
    ...
    [void] Set()
    {
        ...
        Write-Verbose -Message "Setting configuration of the Intune Trusted Root Certificate Policy for iOS with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
    ...
    hidden [IntuneTrustedRootCertificateIOS] AsResult([System.Object] $Values)
    {
        if ($Values -is [IntuneTrustedRootCertificateIOS])
        {
            return $Values
        }
