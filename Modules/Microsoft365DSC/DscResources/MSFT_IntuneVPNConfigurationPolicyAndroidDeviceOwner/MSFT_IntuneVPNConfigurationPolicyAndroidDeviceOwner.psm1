...
[DscResource()]
class IntuneVPNConfigurationPolicyAndroidDeviceOwner : M365DSCResourceBase
{
    [DscProperty()]
    ...
    [IntuneVPNConfigurationPolicyAndroidDeviceOwner] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneVPNConfigurationPolicyAndroidDeviceOwner]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune VPN Policy for Android Device Owner with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.DisplayName -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    $getValue = Get-MgBetaDeviceManagementDeviceConfiguration -All -Filter "DisplayName eq '$($this.Displayname -replace "'", "''")' and isof('microsoft.graph.androidDeviceOwnerVpnConfiguration')" -ErrorAction SilentlyContinue
                }
                #endregion

                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "No Intune VPN Policy for Android Device Owner with Id {$($this.Id)} was found"
    ...
    [void] Set()
    {
        ...
        Write-Verbose -Message "Setting configuration of the Intune VPN Policy for Android Device Owner with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
    ...
    hidden [IntuneVPNConfigurationPolicyAndroidDeviceOwner] AsResult([System.Object] $Values)
    {
        if ($Values -is [IntuneVPNConfigurationPolicyAndroidDeviceOwner])
        {
            return $Values
        }

        $result = [IntuneVPNConfigurationPolicyAndroidDeviceOwner]::new()
