...
[DscResource()]
class IntuneWifiConfigurationPolicyAndroidEnterpriseWorkProfile : M365DSCResourceBase
{
    [DscProperty()]
    ...
    [IntuneWifiConfigurationPolicyAndroidEnterpriseWorkProfile] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneWifiConfigurationPolicyAndroidEnterpriseWorkProfile]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune Wifi Configuration Policy Android Enterprise Work Profile with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.DisplayName -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    $getValue = Get-MgBetaDeviceManagementDeviceConfiguration -All `
                        -Filter "DisplayName eq '$($this.DisplayName -replace "'", "''")' and isof('microsoft.graph.androidWorkProfileWiFiConfiguration') and not isof('microsoft.graph.androidWorkProfileEnterpriseWiFiConfiguration')" `
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "No Intune Wifi Configuration Policy Android Enterprise Work Profile with id {$($this.Id)} was found"
            ...
            Write-Verbose -Message "Found an Intune Wifi Configuration Policy Android Enterprise Work Profile with id {$($resolvedId)}"
            $results = @{
                #region resource generator code
                Id                             = $getValue.Id
    ...
    [void] Set()
    {
        ...
        Write-Verbose -Message "Setting configuration of the Intune Wifi Configuration Policy Android Enterprise Work Profile with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
    ...
    [string] Export()
    {
        ...
        try
        {
            ...
            [array]$getValue = Get-M365DSCExportCachedCollection -Collection 'deviceConfigurations' `
                ...
                -ExcludeODataType 'microsoft.graph.androidWorkProfileEnterpriseWiFiConfiguration' `
                -Filter $this.Filter
            #endregion
    ...
    hidden [IntuneWifiConfigurationPolicyAndroidEnterpriseWorkProfile] AsResult([System.Object] $Values)
    {
        ...
        $result = [IntuneWifiConfigurationPolicyAndroidEnterpriseWorkProfile]::new()
