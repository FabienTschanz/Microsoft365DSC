...
[DscResource()]
class TeamsTenantTrustedIPAddress : M365DSCResourceBase
{
    [DscProperty(Key)]
    [System.ComponentModel.Description('Unique identifier for the IP address to be created.')]
    [System.String] $Identity

    [DscProperty()]
    [System.ComponentModel.Description('Provide a description of the trusted IP address to identify purpose of creating it.')]
    [System.String] $Description

    [DscProperty()]
    [System.ComponentModel.Description('This parameter determines the length of bits to mask to the subnet.')]
    [System.Nullable[System.UInt32]] $MaskBits
    ...
    [TeamsTenantTrustedIPAddress] Get()
    {
        # Declared up front: assigned conditionally below, which class methods reject.
        $nullResult = $null
        if ($this.RequiresPowerShellCore())
        {
            $remote = [TeamsTenantTrustedIPAddress]::new()
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.Identity -ne $this.Identity)
            {
                ...
                Write-Verbose -Message "Getting instance {$($this.Identity)}"
                $instance = Get-CsTenantTrustedIPAddress -Identity $this.Identity -ErrorAction SilentlyContinue
            }
            else
            {
    ...
    [void] Set()
    {
        ...
        {
            Write-Verbose -Message "Removing the Teams Tenant Trusted IP Address with Identity {$($this.Identity)}"
    ...
    [string] Export()
    {
        ...
        try
        {
            [array]$getValue = Get-CsTenantTrustedIPAddress -Filter $this.Filter -ErrorAction Stop
