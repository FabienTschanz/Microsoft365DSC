...
[DscResource()]
class IntuneAntivirusExclusionsPolicyMacOS : M365DSCResourceBase
{
    [DscProperty()]
    ...
    [IntuneAntivirusExclusionsPolicyMacOS] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneAntivirusExclusionsPolicyMacOS]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune Antivirus Exclusions Policy for macOS with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.Name -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Antivirus Exclusions Policy for macOS with Id {$($this.Id)}"
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Antivirus Exclusions Policy for macOS with Name {$($this.DisplayName)}."
            ...
            Write-Verbose -Message "An Intune Antivirus Exclusions Policy for macOS with Id {$($resolvedId)} and Name {$($this.DisplayName)} was found"
    ...
    [void] Set()
    {
        ...
        {
            Write-Verbose -Message "Creating an Intune Antivirus Exclusions Policy for macOS with Name {$($this.DisplayName)}"
    ...
    hidden [IntuneAntivirusExclusionsPolicyMacOS] AsResult([System.Object] $Values)
    {
        if ($Values -is [IntuneAntivirusExclusionsPolicyMacOS])
        {
            return $Values
        }
