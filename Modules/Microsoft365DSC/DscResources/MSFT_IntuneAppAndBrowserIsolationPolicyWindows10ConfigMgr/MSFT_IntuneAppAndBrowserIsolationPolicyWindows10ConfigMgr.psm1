...
[DscResource()]
class IntuneAppAndBrowserIsolationPolicyWindows10ConfigMgr : M365DSCResourceBase
{
    [DscProperty()]
    ...
    [DscProperty()]
    [System.ComponentModel.Description('Install Windows defender application guard (install: Install). Required if AllowWindowsDefenderApplicationGuard is not set to 0.')]
    [ValidateSet('install')]
    ...
    [IntuneAppAndBrowserIsolationPolicyWindows10ConfigMgr] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneAppAndBrowserIsolationPolicyWindows10ConfigMgr]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune App And Browser Isolation Policy for Windows10 ConfigMgr with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.Name -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune App And Browser Isolation Policy for Windows10 Config Mgr with Id {$($this.Id)}"
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune App And Browser Isolation Policy for Windows10 Config Mgr with Name {$($this.DisplayName)}."
            ...
            Write-Verbose -Message "An Intune App And Browser Isolation Policy for Windows10 Config Mgr with Id {$($resolvedId)} and Name {$($this.DisplayName)} was found"
    ...
    [void] Set()
    {
        ...
        {
            Write-Verbose -Message "Creating an Intune App And Browser Isolation Policy for Windows10 Config Mgr with Name {$($this.DisplayName)}"
    ...
    hidden [IntuneAppAndBrowserIsolationPolicyWindows10ConfigMgr] AsResult([System.Object] $Values)
    {
        ...
        $result = [IntuneAppAndBrowserIsolationPolicyWindows10ConfigMgr]::new()
