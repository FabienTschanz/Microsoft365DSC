...
[DscResource()]
class SCFilePlanPropertyCategory : M365DSCResourceBase
{
    [DscProperty(Key)]
    [System.ComponentModel.Description('The Name parameter specifies the unique name of the category.')]
    [System.String] $Name
    ...
    [SCFilePlanPropertyCategory] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [SCFilePlanPropertyCategory]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of SCFilePlanPropertyCategory for $($this.Name)"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.Name -ne $this.Name)
            {
                ...
                if ($null -eq $property)
                {
                    Write-Verbose -Message "SCFilePlanPropertyCategory $($this.Name) does not exist."
            ...
            Write-Verbose "Found existing SCFilePlanPropertyCategory $($this.Name)"
    ...
    [void] Set()
    {
        ...
        Write-Verbose -Message "Setting configuration of SCFilePlanPropertyCategory for $($this.Name)"
        ...
        elseif ($this.Ensure -eq 'Absent' -and $Current.Ensure -eq 'Present')
        {
            try
            {
                $property = Get-FilePlanPropertyCategory | Where-Object -FilterScript { $_.DisplayName -eq $this.Name }
    ...
    hidden [SCFilePlanPropertyCategory] AsResult([System.Object] $Values)
    {
        ...
        $result = [SCFilePlanPropertyCategory]::new()
