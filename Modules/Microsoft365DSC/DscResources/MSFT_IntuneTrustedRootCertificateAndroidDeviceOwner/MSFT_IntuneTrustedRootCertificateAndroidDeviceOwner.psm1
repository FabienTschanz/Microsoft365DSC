...
[DscResource()]
class IntuneTrustedRootCertificateAndroidDeviceOwner : M365DSCResourceBase
{
    [DscProperty()]
    ...
    [IntuneTrustedRootCertificateAndroidDeviceOwner] Get()
    {
        # Declared up front: assigned conditionally below, which class methods reject.
        $getValue = $null
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneTrustedRootCertificateAndroidDeviceOwner]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune Trusted Root Certificate Policy for Android Device Owner with Id {$($this.id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.DisplayName -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    $getValue = Get-MgBetaDeviceManagementDeviceConfiguration -All -Filter "DisplayName eq '$($this.Displayname -replace "'", "''")' and isof('microsoft.graph.androidDeviceOwnerTrustedRootCertificate')" -ErrorAction SilentlyContinue
                }
                #endregion

                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "No Intune Trusted Root Certificate Policy for Android Device Owner with Id {$($this.id)} was found"
            ...
            Write-Verbose -Message "An Intune Trusted Root Certificate Policy for Android Device Owner with id {$($this.id)} and DisplayName {$($this.DisplayName)} was found"
    ...
    [void] Set()
    {
        ...
        Write-Verbose -Message "Setting configuration of the Intune Trusted Root Certificate Policy for Android Device Owner with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
    ...
    hidden [IntuneTrustedRootCertificateAndroidDeviceOwner] AsResult([System.Object] $Values)
    {
        ...
        $result = [IntuneTrustedRootCertificateAndroidDeviceOwner]::new()
