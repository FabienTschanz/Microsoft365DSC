...
[DscResource()]
class IntuneDeviceConfigurationHealthMonitoringPolicyWindows10 : M365DSCResourceBase
{
    [DscProperty()]
    [System.ComponentModel.Description('Enables device health monitoring on the device. Possible values are: notConfigured, enabled, disabled.')]
    [ValidateSet('notConfigured', 'enabled', 'disabled')]
    [System.String] $AllowDeviceHealthMonitoring

    [DscProperty()]
    [System.ComponentModel.Description('Specifies custom set of events collected from the device where health monitoring is enabled')]
    [System.String] $ConfigDeviceHealthMonitoringCustomScope

    [DscProperty()]
    [System.ComponentModel.Description('Specifies set of events collected from the device where health monitoring is enabled. Possible values are: undefined, healthMonitoring, bootPerformance, windowsUpdates, privilegeManagement.')]
    [ValidateSet('undefined', 'healthMonitoring', 'bootPerformance', 'windowsUpdates', 'privilegeManagement')]
    [System.String[]] $ConfigDeviceHealthMonitoringScope
    ...
    [IntuneDeviceConfigurationHealthMonitoringPolicyWindows10] Get()
    {
        if ($this.RequiresPowerShellCore())
        {
            $remote = [IntuneDeviceConfigurationHealthMonitoringPolicyWindows10]::new()
            $remote.FromHashtable($this.InvokeInPowerShellCore('Get'))
            return $remote
        }

        Write-Verbose -Message "Getting configuration of the Intune Device Configuration Health Monitoring Configuration Policy for Windows10 with Id {$($this.Id)} and DisplayName {$($this.DisplayName)}"
        ...
        try
        {
            if (-not $this.ExportedInstance -or $this.ExportedInstance.DisplayName -ne $this.DisplayName)
            {
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Device Configuration Health Monitoring Configuration Policy for Windows10 with Id {$($this.Id)}"
                ...
                if ($null -eq $getValue)
                {
                    Write-Verbose -Message "Could not find an Intune Device Configuration Health Monitoring Configuration Policy for Windows10 with DisplayName {$($this.DisplayName)}"
            ...
            $this.Id = $getValue.Id
            Write-Verbose -Message "An Intune Device Configuration Health Monitoring Configuration Policy for Windows10 with Id {$($this.Id)} and DisplayName {$($this.DisplayName)} was found."
    ...
    [void] Set()
    {
        ...
        {
            Write-Verbose -Message "Creating an Intune Device Configuration Health Monitoring Configuration Policy for Windows10 with DisplayName {$($this.DisplayName)}"
    ...
    hidden [IntuneDeviceConfigurationHealthMonitoringPolicyWindows10] AsResult([System.Object] $Values)
    {
        if ($Values -is [IntuneDeviceConfigurationHealthMonitoringPolicyWindows10])
        {
            return $Values
        }
