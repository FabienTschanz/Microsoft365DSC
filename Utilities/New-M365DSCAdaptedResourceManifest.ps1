...
.DESCRIPTION
    ...
    then replaced with the one from the readme.md of the resource.
...
foreach ($adapted in $manifests)
{
    ...
    $outputPath = Join-Path -Path $moduleRoot -ChildPath "Microsoft365DSC.$name.dsc.adaptedResource.json"
    [System.IO.File]::WriteAllText($outputPath, $adapted.ToJson(), $encoding)
    $written++
}

if ($written -ne $resourceNames.Count)
{
    throw "Wrote $written adapted resource manifests but the manifest exports $($resourceNames.Count) resources."
}

Write-Host "[adapted] Wrote $written adapted resource manifests to $moduleRoot" -ForegroundColor Green
